...
| **4 — GUI MVP** | `phase-4` | **Implemented — all exit criteria CI-enforced** | `kestrel-gui`: Slint shell (ADR 0001), sandboxed `wry` viewport + `kestrel-cid://`, composer, tray, notifications, theme | Threat-model §7 webview test matrix green; cold start < 200 ms; CSP verified on every load |
