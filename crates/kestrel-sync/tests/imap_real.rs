//! Real-world IMAP integration tests.
//!
//! Every test is `#[ignore]`d and gated by `KESTREL_IMAP_INTEGRATION=1`.
//! Credentials are loaded from env vars — **never** committed.
//!
//! Generic IMAP test env vars:
//!   - `KESTREL_IMAP_INTEGRATION=1`
//!   - `KESTREL_IMAP_HOST` (e.g., `imap.migadu.com` or `imap.gmail.com`)
//!   - `KESTREL_IMAP_PORT` (default: 993)
//!   - `KESTREL_IMAP_EMAIL`
//!   - `KESTREL_IMAP_PASSWORD`
//!
//! Gmail-specific `OAuth2` test (requires separate setup):
//!   - `KESTREL_GMAIL_INTEGRATION=1`
//!   - `KESTREL_GMAIL_REFRESH_TOKEN`, `KESTREL_GMAIL_CLIENT_ID`, `KESTREL_GMAIL_CLIENT_SECRET`
...
fn integration_enabled() -> bool {
    std::env::var("KESTREL_IMAP_INTEGRATION").is_ok_and(|v| v == "1")
}
...
#[tokio::test]
#[ignore = "real: KESTREL_IMAP_INTEGRATION=1 + env vars required"]
async fn imap_real_password_auth() {
    if !integration_enabled() {
        eprintln!("skipping: KESTREL_IMAP_INTEGRATION not set");
        return;
    }

    let _ = tracing_subscriber::fmt::try_init();

    let host = std::env::var("KESTREL_IMAP_HOST").unwrap_or_else(|_| "imap.gmail.com".into());
    let port: u16 = std::env::var("KESTREL_IMAP_PORT")
        .ok()
        .and_then(|p| p.parse().ok())
        .unwrap_or(993);
    let email = env_or_skip("KESTREL_IMAP_EMAIL");
    let password = env_or_skip("KESTREL_IMAP_PASSWORD");

    eprintln!("imap_real: connecting to {host}:{port} as {email}");
    ...
    eprintln!("imap_real: connected and authenticated");
    ...
    // 2. LIST folders.
    {
        ...
        eprintln!("imap_real: found {folder_count} folders");
    ...
    // 3. SELECT INBOX.
    {
        ...
        eprintln!("imap_real: INBOX selected");
    }
    ...
    // 4. FETCH 5 ENVELOPEs.
    {
        ...
        eprintln!("imap_real: fetched {fetch_count} envelopes");
        assert!(fetch_count > 0, "No envelopes fetched");
    }

    // 5. Clean disconnect.
    session.logout().await;
    eprintln!("imap_real: test passed");
}
